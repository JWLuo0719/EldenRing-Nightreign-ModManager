...
最终确认原存档并未整体损坏：女爵选择了“更多服装”提供、游戏本体不存在的服装，
因此模型为空；换回默认或本体已有服装后立即恢复。完整整合方案此前已在新角色上
跑通，后续以修正服装后的原存档继续验收。

2026-08-01 恢复同一原始存档并使用修正后的社区 Profile 后获得了更精确的结论：
游戏服务器登录成功，Steam 好友邀请可用；除女爵外的其他角色能够正常显示。只有
女爵仍不可见，且切换到其他角色后不能再切回女爵。因此“原存档人物异常”应收窄为
女爵专属状态/资源/参数异常，不能再表述成全部人物渲染失败。Nexus 578 当前公开 Bug
列表没有同名已知问题，且作者明确不支持 Seamless 或非官方合并，仍需本地单变量对照。

随后关闭 MMV/Weapons、恢复相同存档，只加载 Seamless + 602，女爵故障仍完全复现。
本轮生成 Profile 和 ME3 日志均证明只有 `nrsc.dll` 与 602；602 只覆盖 item/menu 两个
消息包。标题页脚中的 MMV/Weapons 署名来自 602 自带菜单文本，不代表玩法 package
仍在运行。`女爵去除面罩.disabled` 只有 5030/5130/5230/5330/5530 五个 body parts，
未被 Profile 或运行日志引用，也没有 regulation；因此直接文件残留不成立。纯 Seamless
测试时标题页脚署名消失，进一步验证没有隐藏 package；用户随后换回本体已有服装，
女爵正常显示。根因是无效自定义服装选择，不是女爵解锁标志或存档主体损坏。
...
| 管理器生成副本中移除 Redirector，改用现有 nrsc.dll | 本机 Spacewar 单人阶段验收通过 | 602、地图/敌人、武器、登录和邀请已通过；待第二位玩家实际加入同房 |
...
本次阶段验收使用的恢复点是“修正女爵服装后的同源 `NR0000.co2` + MMV/Weapons 社区模式 +
602 + Seamless”，
活动存档与 `.bak` 的 SHA-256 都是
`F03D51FA08B3D92EAF1B72B0D365408899D832420DA02202694ACC247E72E379`。MMV/Weapons
与 602 已重新启用，女爵面具 Mod 已恢复到 `Game\mods` 下但继续保持 `.disabled`。
下一轮验证女爵与其他角色显示、MMV 地图/敌人/武器、中文和联机；游戏内操作仍由
用户人工完成并发送截图。不要自动删除原进度。

上述恢复点现已完成阶段验收。用户截图和两轮 ME3 日志共同确认：

1. MMV + Weapons + 602 + `nrsc.dll` 同时加载时，新增武器、中文名称/战技/词条和
   有效服装下的女爵模型正常；服务器登录与 Steam 好友邀请可用；
2. 关闭 602 后，新增武器仍存在但文本变成 `?WeaponName?`、`ArtsName(5170)`、
   `AttachEffectName(9040600)`，证明 602 是独立文本层；
3. 关闭 602 的同一轮实际进入本体原先没有的白金之子主城，证明 MMV 地图/敌人内容
   确实加载，不能再只把标题页脚或 Profile 生成当作成功证据。

阶段结束时保留 MMV + Weapons 启用、602 关闭、女爵面具 Mod `.disabled`。验收存档
双文件哈希为
`C658A54655F621E78DFE20C451A8D584961D104BEFD979B1CD0C827D60AB96CD`，已备份到
`backups/saves/manual-stage1-mmv-validated-20260801-2230/76561199403037768/`。
第二位玩家尚未真正加入同房，因此“双人联机实战”仍是后续验收项。

新对话不再重复本专项。2026-08-01 已完成第一轮衔接功能：管理器能区分纯 parts
“服装替换”和含 regulation 的“扩展服装”，统计同名 `_l` 队友视角资源，扩展服装
ZIP 与首次外部目录注册默认停用，并在停用/删除/配置方案切换前提醒先换回本体服装。主要界面已采用
“资源型 Mod / 功能插件 / 玩法数据文件 / 启动配置 / 联机一致性清单”等玩家术语，
技术原词保留在折叠高级详情中。

本机只读样本结果：女爵去面罩为 5 个本机 parts、无 regulation、缺 5 个 `_l`；
SkinOverhaul 当前目录为 228 个本机 parts 与 228 个 `_l` 全配对，含 regulation 和
`01_Online.bat`。管理器没有运行该脚本。下一步是用更多真实服装 ZIP 验证半套 `_l`、
不同部位命名和冲突组合，而不是重新验证 MMV/602。

2026-08-02 的“更多服装未加载”实测已定位为外部注册路径失效：注册项仍是带空格的
`Skin Overhaul`，磁盘实际目录为 `SkinOverhaul`，因此本轮生成 Profile 与 ME3 日志
完全没有该 package。管理器已新增失效路径标记、重新定位与配置方案 ID 迁移，并在
后端拒绝静默跳过。重新定位后仍不能与当前 MMV 直接同开：两个 package 各有一份且
哈希不同的 `regulation.bin`。现在启动前检查、普通启动与诊断启动都会硬阻止这种组合；
后续应先做 Skin-only 验收，或得到适配当前 MMV 版本的参数级合并产物。
...
- 602 常见的根级或 `zhocn\` 两文件布局会自动规范成标准 `msg\zhocn\`，
  多套/半套布局会整包回滚。
- 服装 parts / `_l` 配对 / regulation / 外观 ID 结构检查，扩展服装 ZIP 默认停用，
  以及启停和方案切换的存档外观安全提示；作者联机脚本只识别、不执行。
- 玩家化术语与可折叠高级技术对照。
